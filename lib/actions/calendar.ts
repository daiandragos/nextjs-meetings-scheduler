...
export async function disconnectGoogleAccount(
  ...
  // revoke token
  if (account.accessToken) await revokeGoogleToken(account.accessToken);
...
export async function setDefaultCalendarAccount(
  ...
  // set current as default, others as non-default
...
export async function getBookingAttendeeStatuses(
  ...
  const bookingWithEvents = bookings.filter((b) => b.googleEventId);

  await Promise.all(
    bookingWithEvents.map(async (booking) => {
      if (booking.googleEventId) {
        const { hostStatus, guestStatus } = await getEventAttendeesStatuses(
          account,
