...
import { computeAvailableDates } from "../availability";
    ...
    await Promise.all(
      existingBookings
        ...
        .map(async (booking) => {
          ...
          try {
            const status = await getEventAttendeesStatus(
              defaultAccount,
...
async function checkSlotAvailable(
  ...
  return !existingBookings.some((booking) => {
    if (declinedBookingIds.has(booking._id)) return false;
    const bookingStart = parseISO(booking.startTime);
    const bookingEnd = parseISO(booking.endTime);
    return startTime < bookingEnd && endTime > bookingStart;
  });
