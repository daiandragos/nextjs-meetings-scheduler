...
export function AvailabilityCalendar({
  ...
  const allEvents = [...events, busyBlocks, bookedBlocks];
  ...
  //   get status indicator based on guest response
        ...
        <DialogContent className="sm: max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <div
                className={`h-3 w-3 rounded-full ${selectedBooking?.attendeeStatus === "declined" ? "bg-red-500" : selectedBooking?.attendeeStatus === "tentative" ? "bg-amber-500" : selectedBooking?.attendeeStatus === "accepted" ? "bg-green-600" : "bg-gray-500"}`}
              >
                Meeting Details
              </div>
            </DialogTitle>
          ...
          {selectedBooking && (
            <div className="space-y-4 pt-2">
              <div
                className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-sm font-medium ${selectedBooking?.attendeeStatus === "declined" ? "text-red-700" : selectedBooking?.attendeeStatus === "tentative" ? "text-amber-700" : selectedBooking?.attendeeStatus === "accepted" ? "text-green-700" : "text-gray-700"}}`}
              >
                ...
                {!selectedBooking.attendeeStatus && "Guest Response Unknown"}
              </div>
              ...
              <div className="flex items-start gap-3">
                <Mail className="h-5 w-5 mt-0.5 text-muted-foreground"></Mail>
                <div>
                  <a
